...
![image](https://github.com/user-attachments/assets/4af73eff-5586-43ca-b225-4ff8c471e6c5)
...
# Отправка писем
При отправке письма, проверяется, в нашем ли домене находится получатель. Если да, то письмо помечается внутренним и сохраняется с БД получателя. Если же получатель находится в другом домене, то письмо помечается внешним и отправляется на сервис SMTP для отправки на другой домен
