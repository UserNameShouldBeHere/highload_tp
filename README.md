...
![image](https://github.com/user-attachments/assets/38ebaaba-8ce3-4034-b9ce-2d424f4aa512)
...
### Отправка писем
При отправке письма, проверяется, в нашем ли домене находится получатель. Если да, то адресат помечается внутренним, и письмо сохраняется с БД получателя. Если же получатель находится в другом домене, то адресат помечается внешним, и письмо отправляется на сервис SMTP для отправки на другой домен
...
### Получение письма
Для получения письма сначала будут взяты заголовки и ссылка на тело из БД. Затем получено само тело, В котором могут быть вложения. На месте вложений будут ссылки на S3. Заголовки и тело письма в виде единого целого уже будет отправлено пользователю. Также, при получении проверяется, является ли адресат внешним. Если он из стороннего сервиса, то он помечается внешним 
...
|-|-|
|postgres|master-slave репликация инструментами postgres с помощью WAL|
...
|kubenode sqlite3|собственный|1xAMD EPYC 7313P / 32GB / 20xHDD24TB / 10Gb/s|21|12800|
