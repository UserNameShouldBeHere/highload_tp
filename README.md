...
- для Cassandra: https://pkg.go.dev/github.com/gocql/gocql
...
|-|-|-|
|SHA256|хэширование паролей|надежный алгоритмы, не допускающий взлом перебором, что защитит при утечки базы данных, устойчивый к коллизиям|
...
|tls|шифрование трафика в сети|алгоритм шифрования, используемый при https соединении|
|sql запросы|поиск / сортировка / группировка|sql запросы в базе данных для сортировки писем по времени, группировке по папкам, поиска по названию|
...
|Cassandra|основная БД для хранения всей информации|высокомасштабируемая распределенная система хранения для хранения огромных массивов данных|
...
|Prometheus + Grafana|сбор и вузуализация метрик|Prometheus необходим для сбора метрик с различных сервисов, Grafana позволяет визуализировать данные с помощью различных графиков|
...
|JavaScript|клиентская сторона|необходим для разработки клиентской части приложения|
